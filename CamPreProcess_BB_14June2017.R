...
#Temp test
#test4 <- test2
#write.csv(test4, file = "temp1")

temp <- as.data.frame(read.csv("testsample.csv"))
temp$X <- NULL
str(temp)
temp$instdate <- as.Date(temp$instdate)
temp$remdatefill <- as.Date(temp$remdatefill)
temp$folderdateend <- as.Date(temp$folderdateend)
temp$folderdatestart <- as.Date(temp$folderdatestart)
temp$DateTimeOriginal <- ymd_hms(temp$DateTimeOriginal)

image <- temp[, c("cam", "site", "DateTimeOriginal")]
image$folderdateend <- image$DateTimeOriginal
names(image) <- c("cam", "site", "folderdatestart", "folderdateend")
str(image)

temp2 <- temp
temp3 <- temp2[!is.na(temp2$DateTimeOriginal), ] #take out .txt files for image work (will be doing eventually)

temp4 <- temp3 %>%
  group_by(site, instdate) %>% 
  mutate(folderdatestart = ifelse(is.na(folderdatestart), instdate,folderdatestart))%>%
  mutate(folderdatestart = as.Date(folderdatestart, origin = "1970-01-01")) %>% data.frame()

######Trevor sample code for lag and, filling NA dates, and correcting back to a date format
test2<-test[-(1:335),]
...
###############################Old code for main image data frame, extract info as needed
