...
```
Side note: To check file types (see if .ds_stores exist or other files when intentionally not including hidden files (all.files = FALSE)), would it be logical to do a str_split based on "." to create a column of file types? Should we be creating that column anyways?

Things to check: Is cameradate blank? (blanks <- images[images$cameradate == "", ]) Why use "[]" again?

Querying for blank instances of images: (empty <- rownames(table(images$cameradate)[which(table(images$cameradate) == 1)]))

Checking for non .jpgs (wtf<- filter(images, !grepl(".JPG", image)))
...
```
See "brook_thesiscodeall_6may2016.Rmd" for code to check differences between the image server and a backup file (not sure if completed) and some test code for examinining images pre/post a certain date.